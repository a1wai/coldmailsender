 ...
 * @param {boolean} [message.hasOneClickUnsubscribe] RFC 8058 HTTPS opt-out.
...
export function analyseMessage({
  ...
  if (hasUnsubscribe && !hasOneClickUnsubscribe) {
    add(
      'medium',
      'Opt-out is reply-only',
      'Google, Yahoo and Apple all expect one-click unsubscribe (RFC 8058) on outreach mail, and that is defined over HTTPS — a mailto: link does not count. It also protects you: someone who can leave in one tap does that instead of reporting spam.',
      'Deploy this app to a public URL, or set UNSUBSCRIBE_BASE_URL, and the built-in /api/unsubscribe endpoint takes care of it.',
    );
  }
...
/**
 * The advice that actually moves the needle, ordered by impact.
 *
 * The numbers here come from the sender requirements Google and Yahoo
 * introduced in February 2024 (Microsoft and Apple followed with their own
 * versions), which is the rulebook every mailbox provider now scores against.
 * The headline thresholds:
 *
 *   - Spam complaint rate below 0.10%. At 0.30% enforcement starts, and
 *     0.30% is the point of failure, not a target to aim at.
 *   - SPF and DKIM both required, with DMARC on top for bulk senders.
 *   - One-click unsubscribe (RFC 8058) on marketing mail, honoured within
 *     two days.
 *   - Valid forward-confirmed reverse DNS, TLS on submission, and RFC 5322
 *     conformant headers.
 *
 * The formal "bulk sender" line is 5,000 messages a day to one provider, so
 * most users of this app sit under it. That exempts you from the rules being
 * *enforced* as a pass/fail gate — it does not exempt you from being scored on
 * them, which is why the same list still applies at fifty a day.
 */
export const DELIVERABILITY_PLAYBOOK = [
  {
    title: 'Send from your own domain, not @gmail.com',
    detail:
      'This is the answer in most cases. A free mailbox cannot be DKIM-signed as yours, so it can never pass DMARC ' +
      'alignment on a domain you control, and it carries the shared reputation of every other account on that ' +
      'provider. Cold outreach from a consumer Gmail address is filtered on sight by a lot of corporate mail ' +
      'gateways. A domain with Google Workspace behind it (about $7/month) is the single biggest change available, ' +
      'and nothing else on this list fully compensates for skipping it.',
    impact: 'huge',
  },
  {
    title: 'Use a separate domain from your main one',
    detail:
      'Cold outreach damages sender reputation even when done well — some recipients will always hit the spam ' +
      'button. Send it from a lookalike domain (yourcompany-mail.com) so a bad month never affects the invoices ' +
      'and contracts going out from your primary domain.',
    impact: 'high',
  },
  {
    title: 'Set up SPF, DKIM and DMARC — and check alignment',
    detail:
      'These three DNS records prove the mail really came from you. Publishing them is not enough on its own: the ' +
      'domain in your From: header has to match the domain SPF or DKIM authenticated, or DMARC fails anyway. Start ' +
      'DMARC at p=none while you read the reports, then move to p=quarantine. The DNS check on this page reports ' +
      'all three.',
    impact: 'huge',
  },
  {
    title: 'Offer one-click unsubscribe',
    detail:
      'Since 2024 Google, Yahoo and Apple all expect a List-Unsubscribe-Post header on promotional mail, and a ' +
      'mailto: link alone does not satisfy it — RFC 8058 one-click is HTTPS only. It also protects you: someone who ' +
      'can leave in one tap does that instead of pressing "report spam", and complaint rate is the metric that ' +
      'actually gets you blocked. This app hosts the endpoint for you at /api/unsubscribe.',
    impact: 'high',
  },
  {
    title: 'Keep the complaint rate under 0.1%',
    detail:
      'One spam complaint per thousand messages is the ceiling worth planning around; 0.3% is where providers start ' +
      'actively filtering or blocking you. That works out at roughly one complaint per 1,000 sends — so tight ' +
      'targeting matters far more than volume. Reputation falls quickly and recovers slowly.',
    impact: 'high',
  },
  {
    ...
    detail:
      ...
      '5–10 a day, hold a predictable daily volume, and roughly double each week over four to six weeks. Sudden ' +
      'spikes are one of the most reliable ways to get filtered, even from a well-authenticated domain.',
    impact: 'high',
  },
  {
    title: 'Get replies',
    detail:
      'Replies are the strongest positive reputation signal there is — stronger than opens, which are increasingly ' +
      'unmeasurable anyway. A short message that asks one easy question beats a polished pitch nobody answers.',
    impact: 'high',
  },
  {
    title: 'Keep it plain, short and un-tracked',
    detail:
      'No images, no tracking pixel, no HTML newsletter layout, one link at most. Tracking pixels and redirect ' +
      'domains are themselves a spam signal on a first contact. This app sends minimal HTML with a plain-text ' +
      'alternative and no tracking for exactly this reason.',
    impact: 'medium',
  },
  {
    title: 'Verify addresses before sending',
    detail:
      'Bouncing off dead addresses is a strong negative signal, and abandoned mailboxes get recycled into spam ' +
      'traps — hitting one can get a domain blocklisted outright. Scraped addresses are exactly the kind that go ' +
      'stale, so keep the bounce rate under 2% and drop anything that hard-bounces immediately.',
    impact: 'medium',
  },
  {
    title: 'Never buy a list, and honour every opt-out',
    detail:
      'A purchased list is the fastest route to a spam trap. And once someone opts out, mailing them again is both ' +
      'illegal in most jurisdictions and the most likely single message to be reported. The opt-out list in this ' +
      'app is checked before every send.',
    impact: 'high',
  },
];

/**
 * Suggested daily ceiling by week, for a fresh sending address.
 *
 * Deliberately conservative — the cost of ramping too slowly is a few extra
 * days, and the cost of ramping too fast is a burnt domain.
 */
export const WARMUP_SCHEDULE = [
  { week: 1, perDay: 10, note: 'Only addresses likely to reply. Answer every reply by hand.' },
  { week: 2, perDay: 20, note: 'Hold it steady — a predictable volume matters more than the number.' },
  { week: 3, perDay: 40, note: 'Watch for bounces; stop and fix before increasing again.' },
  { week: 4, perDay: 80, note: 'Only continue if replies are coming and nothing is bouncing.' },
  { week: 5, perDay: 150, note: 'Approaching normal volume.' },
  { week: 6, perDay: 300, note: 'Full volume, still under the Gmail 500/day cap.' },
];

/**
 * The daily cap to suggest for an address that has been sending for `days`.
 *
 * @param {number} days How long this address has been used for outreach.
 * @returns {{ perDay: number, week: number, note: string }}
 */
export function recommendedDailyCap(days) {
  const week = Math.max(1, Math.ceil((Number(days) || 0) / 7) || 1);
  return WARMUP_SCHEDULE.find((entry) => entry.week >= week) || WARMUP_SCHEDULE[WARMUP_SCHEDULE.length - 1];
}
