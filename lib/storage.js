...
export function useStoredState(key, initialValue, options = {}) {
  ...
  useEffect(() => {
    ...
    // Never write an untouched default over a stored value. This matters when
    // `kind` changes: the switch and the load of the new location happen in
    // separate commits, and without this the default would clobber whatever
    // was remembered in the meantime.
