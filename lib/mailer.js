 ...
 * @param {object} [params.compliance] `{ postalAddress, unsubscribeEmail, unsubscribeUrl, appendFooter, appOrigin }`
 *                                     `appOrigin` is this deployment's public
 *                                     URL; supplying it generates a per-recipient
 *                                     one-click opt-out link automatically.
 */
export async function sendMail({
  ...
  // Give every recipient their own one-click opt-out link when the deployment
  // knows its own public origin. This is what turns `List-Unsubscribe` from a
  // mailto-only courtesy into RFC 8058 one-click, which Gmail, Yahoo and Apple
  // all weigh when choosing between the inbox and the spam folder.
  const footer = buildComplianceFooter(
    {
      ...
      unsubscribeUrl:
        ...
        buildUnsubscribeUrl({ origin: compliance.appOrigin, email: recipient, sender: creds.user }),
    },
    creds,
  );
